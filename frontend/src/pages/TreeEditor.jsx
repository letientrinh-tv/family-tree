...
// Estimated PersonNode dimensions — used only for initial approximation
const NODE_W = 160;
const NODE_H = 64;
          ...
          {!isAlive && (
            <div>
              ...
              <DatePicker
                ...
                onChange={(val) => setForm((p) => ({ ...p, death_date: val }))}
                placeholder="Năm mất"
              />
            </div>
          )}
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <strong style={{ color: "#3C2415" }}>{src.full_name}</strong> —{" "}
          <strong style={{ color: "#3C2415" }}>{tgt.full_name}</strong>
        </p>
        {row(
          `↑  ${src.full_name} là cha/mẹ của ${tgt.full_name}`,
          "#F5F0E8",
          "#8B4513",
          () => onConfirm("parent_child", src.id, tgt.id),
        )}
        {row(
          `↑  ${tgt.full_name} là cha/mẹ của ${src.full_name}`,
          "#F5F0E8",
          "#8B4513",
          () => onConfirm("parent_child", tgt.id, src.id),
        )}
        {row(
          `♥  ${src.full_name} và ${tgt.full_name} là vợ/chồng`,
          "#FEF0F6",
          "#D946A8",
          () => onConfirm("spouse", src.id, tgt.id),
        )}
        <button
          onClick={onClose}
          className="btn-secondary"
      ...
      >
        ...
        >
          {filtered.length === persons.length
            ...
            : `${filtered.length} / ${persons.length} kết quả`}
        </div>
        ...
        <div style={{ flex: 1, overflowY: "auto" }}>
          {filtered.length === 0 ? (
            ...
            filtered.map((person, idx) => {
              ...
              const birthY = person.birth_date
                ? person.birth_date.split("-")[0] || person.birth_date
                : null;
              const deathY = person.death_date
                ? person.death_date.split("-")[0] || person.death_date
                : null;
              const lifeInfo = [
                birthY && `Sinh ${birthY}`,
                deathY && `Mất ${deathY}`,
              ...
              return (
                <div
                  ...
                  style={{
                    ...
                    gap: "12px",
                    padding: "9px 14px",
                    background: idx % 2 === 0 ? "#FDFAF5" : "#F9F5EE",
                ...
                >
                  ...
                  >
                    {person.photo_url ? (
                      ...
                      <span style={{ color: meta.color, fontSize: "1.1rem" }}>
                        {meta.icon}
                      </span>
                    )}
                  </div>
...
function TreeEditorInner() {
  ...
  // Build nodes and edges from data using junction nodes for couple connections
  const buildGraph = useCallback((personList, relList, onAddRelative) => {
    ...
    const spouseRels = relList.filter((r) => r.relationship_type === "spouse");
    const junctionNodes = [];
    const junctionEdges = [];
    const parentToJunction = {}; // personId → junctionNodeId
    ...
    spouseRels.forEach((rel) => {
      ...
      const x1 = p1.position_x || 0;
      const x2 = p2.position_x || 0;
      const leftP = x1 <= x2 ? p1 : p2;
      const rightP = x1 <= x2 ? p2 : p1;

      const jId = `j_${rel.id}`;
      // Center the junction dot on the midpoint of the horizontal spouse line
      const jX = (leftP.position_x + NODE_W + rightP.position_x) / 2 - 1;
      const jY = (leftP.position_y || 0) + NODE_H / 2 - 1;
      ...
      // Horizontal lines: left parent → junction, right parent → junction
      junctionEdges.push({
        id: `je_L_${rel.id}`,
        source: String(leftP.id),
        target: jId,
        type: "straight",
        sourceHandle: "right",
        targetHandle: "left",
      ...
      parentToJunction[rel.person1_id] = jId;
      parentToJunction[rel.person2_id] = jId;
    });

    const personNodes = personList.map((p) => ({
      id: String(p.id),
      type: "person",
      position: { x: p.position_x || 0, y: p.position_y || 0 },
      data: { ...p, onAddRelative },
    }));

    // Deduplicate: if both parents share a junction, only create one edge per child
    const processedPCEdges = new Set();
    const pcEdges = relList
      .filter((r) => r.relationship_type === "parent_child")
      .filter((r) => {
        const src = parentToJunction[r.person1_id] || String(r.person1_id);
        const key = `${src}_${r.person2_id}`;
        if (processedPCEdges.has(key)) return false;
        processedPCEdges.add(key);
        return true;
      })
      .map((r) => {
        const jId = parentToJunction[r.person1_id];
        return {
          id: String(r.id),
          source: jId || String(r.person1_id),
          ...
          data: { relationship_type: "parent_child" },
        };
      });

    return {
      newNodes: [...personNodes, ...junctionNodes],
      newEdges: [...pcEdges, ...junctionEdges],
    };
  }, []);
  ...
  const realignJunctions = useCallback(() => {
    ...
    const updated = allNodes.map((node) => {
      ...
      if (!rel) return node;
      const p1Node = allNodes.find((n) => n.id === String(rel.person1_id));
      const p2Node = allNodes.find((n) => n.id === String(rel.person2_id));
      if (!p1Node || !p2Node || !p1Node.width || !p2Node.width) return node;
      const leftNode = p1Node.position.x <= p2Node.position.x ? p1Node : p2Node;
      const rightNode =
      ...
      if (
        ...
        hasChange = true;
        return { ...node, position: { x: jX, y: jY } };
      }
      return node;
    });
    if (hasChange) setNodes(updated);
    ...
    apiClient
      ...
      .catch((err) => {
        if (err.response?.status === 404 || err.response?.status === 403) {
          ...
          toast.error(
            "Lỗi tải dữ liệu: " + (err.response?.data?.detail || err.message),
          );
        }
      })
  ...
  const handleNodeDragStop = useCallback(
    async (event, node) => {
      ...
      setTimeout(realignJunctions, 50);
      try {
  ...
  const handleDeletePerson = useCallback(
    async (personId) => {
      ...
      try {
        ...
        setUndoStack((prev) => [
          ...prev.slice(-9),
          { person: personToDelete, relationships: relsToDelete },
        ]);
  ...
  const handleUndo = useCallback(async () => {
    ...
    try {
      ...
      for (const rel of rels) {
        try {
          const r = await apiClient.post(`/trees/${treeId}/relationships`, {
            person1_id:
              rel.person1_id === person.id ? newPerson.id : rel.person1_id,
            person2_id:
              rel.person2_id === person.id ? newPerson.id : rel.person2_id,
            relationship_type: rel.relationship_type,
          });
          newRels.push(r.data);
        } catch {
          /* quan hệ với node đã xóa, bỏ qua */
        }
      }
      ...
      toast.error(
        "Lỗi khôi phục: " + (err.response?.data?.detail || err.message),
      );
    }
  }, [undoStack, treeId]);
  ...
  const handleAddPerson = useCallback(
    async (formData, mode, relativeOf) => {
      // Calculate position
      let posX = 100;
      let posY = 100;
      if (relativeOf) {
      ...
      try {
        ...
        if (relativeOf && mode !== "first" && mode !== "standalone") {
          ...
          const relRes = await apiClient.post(
        ...
        toast.error(
          "Lỗi thêm thành viên: " + (err.response?.data?.detail || err.message),
        );
        throw err;
      }
    },
  ...
  const handleAutoLayout = useCallback(async () => {
    ...
    const coupleParents = {};
    Object.keys(coupleData).forEach((k) => {
      coupleChildren[k] = [];
      coupleParents[k] = [];
    });
    Object.entries(coupleData).forEach(([key, { childIds }]) => {
      const childKeys = [
        ...new Set(
          childIds.map((c) => personCouple[c]).filter((k) => k && k !== key),
        ),
      ];
    ...
    const rootCouples = Object.keys(coupleData).filter(
      (k) => coupleParents[k].length === 0,
    );
    ...
    persons.forEach((p, i) => {
      if (!positions[p.id])
        positions[p.id] = { x: i * STEP_X - totalW / 2, y: -STEP_Y };
    });
    ...
    // Re-align junction nodes after layout renders
    setTimeout(realignJunctions, 150);

    // Lưu vị trí lên server
    updated.forEach((p) => {
      apiClient
        .put(`/persons/${p.id}/position`, {
          position_x: p.position_x,
          position_y: p.position_y,
        })
        .catch(() => {});
    });
  }, [persons, relationships, fitView, realignJunctions]);
    ...
    >
              ...
              >
                ...
                >
                  ...
                  <div style={{ minWidth: 0 }}>
                    ...
                    <div style={{ fontSize: "0.67rem", color: "#7a5c3e" }}>
                      {persons.length} thành viên · {relationships.length} quan
                      hệ
                    </div>
                  </div>
                </div>
                ...
                <div style={{ padding: "8px 10px" }}>
                  <button
                    ...
                    style={{
                      width: "100%",
                      background: "linear-gradient(135deg, #6b3a7d, #9b4dca)",
                      color: "#F5F0E8",
                      border: "none",
                      borderRadius: "5px",
                      padding: "7px 4px",
          ...
          <Panel position="top-right">
            ...
            >
              {persons.length === 0 ? (
                <button
                  onClick={() =>
                    setAddModal({ mode: "first", relativeOf: null })
                  }
                  className="btn-primary"
                ...
                >
                  ...
                  >
                    Click node để xem & sửa
                    <br />
                    Kéo từ handle ● sang node
                    <br />
                    khác để tạo quan hệ tự do
                  </div>
                </div>
              )}
            </div>
      ...
      {selectedPerson && (
        <PersonSidebar
          ...
          onAddRelative={(mode) =>
            setAddModal({ mode, relativeOf: selectedPerson })
          }
        />
      )}
